...
impl Expression {
    pub fn walk(&self, cb: &mut impl FnMut(&Expression) -> bool) {
        ...
        match self {
            ...
            // unary
            Expression::Neg(a) | Expression::Sqrt(a, _) => a.walk(cb),
            // no sub-expressions
    ...
    pub fn evaluate(
        &self,
        var_values: &HashMap<Variable, Concrete>,
    ) -> Result<Concrete, ResolveErr> {
        // TODO: support multiple results in return set
        match self {
            Expression::Sum(a, b) => match (a.evaluate(var_values)?, b.evaluate(var_values)?) {
                (Concrete::Rational(a), Concrete::Rational(b)) => Ok(Concrete::Rational(a + b)),
                _ => todo!("{:?} + {:?}", a, b),
            },
            Expression::Difference(a, b) => {
                match (a.evaluate(var_values)?, b.evaluate(var_values)?) {
                    (Concrete::Rational(a), Concrete::Rational(b)) => Ok(Concrete::Rational(a - b)),
                    _ => todo!("{:?} - {:?}", a, b),
                }
            }
            Expression::Product(a, b) => match (a.evaluate(var_values)?, b.evaluate(var_values)?) {
                (Concrete::Rational(a), Concrete::Rational(b)) => Ok(Concrete::Rational(a * b)),
                _ => todo!("{:?} * {:?}", a, b),
            },
            Expression::Quotient(a, b) => {
            ...
            Expression::Neg(a) => match a.evaluate(var_values)? {
                Concrete::Rational(a) => Ok(Concrete::Rational(-a)),
                _ => todo!("-{:?}", a),
            },

            Expression::Power(a, b) => match (a.evaluate(var_values)?, b.evaluate(var_values)?) {
                (Concrete::Rational(a), Concrete::Rational(b)) => {
                    use num::ToPrimitive;
                    match b.to_i32() {
            ...
            Expression::Variable(v) => {
                if let Some(c) = var_values.get(v) {
                    Ok(c.clone())
                } else {
    ...
    pub fn simplify(&mut self) {
        ...
        match self {
            ...
            // unary
            Expression::Neg(a) | Expression::Sqrt(a, _) => a.simplify(),
            // no sub-expressions
            Expression::Integer(_) | Expression::Rational(_, _) | Expression::Variable(_) => {}
        }
